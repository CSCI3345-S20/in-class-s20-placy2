...
class TaskList1Spec extends PlaySpec with GuiceOneServerPerSuite with OneBrowserPerSuite with HtmlUnitFactory {
  "Task list 1" must {
    "login and access functions" in {
      ...
      textField("username-login").value = "placy"
      ...
      eventually {
        ...
        findAll(cssSelector("li")).toList.map(_.text) mustBe List("Testing", "Software Engineering", "Music History", "Sleep")
      }
    }
    //Keep testing w user adds, message creates, deletes, etc.
  }
}